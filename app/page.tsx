...
import { AlertCircle, ChevronRight, Search, Plus, RefreshCw, Archive, Loader2 } from "lucide-react";
...
export default function NutritionistDashboardPage() {
  ...
  // Replaces the initial mock personas with her real client list from
  // Supabase. Deliberately a full replace (setClients), not a merge —
  // the mock array's job was only ever to make the app browsable before
  // any real backend existed; now that real data exists, it's the source
  // of truth. NOTE: the 4 original mock personas are NOT currently
  // seeded into Supabase, so they will disappear from this list once
  // this fetch runs. Flagged for you to decide whether to seed them for
  // demo purposes, per the earlier "keep seed data too" decision.
  useEffect(() => {
    let cancelled = false;
    ...
    async function loadClients() {
      ...
      if (error) {
        setLoadError("Couldn't load your client list. Pull to refresh, or check your connection.");
        setLoading(false);
        return;
      }
  ...
  return (
    <div>
      ...
      <div className="px-5">
        ...
        {loading && (
          <div className="flex items-center justify-center gap-2 py-10 text-moss-400 text-sm">
        ...
        {!loading && loadError && (
          <p className="text-sm text-clay-600 text-center py-8">{loadError}</p>
        )}
